...
#Region "CONECTAR"
    Public Sub Conectar()
        Try
            ...
            consulta = "SELECT *FROM quipux_circular"
            adaptador = New MySqlDataAdapter(consulta, conexion1)
            datos = New DataSet
            adaptador.Fill(datos, "quipux_circular")
            DgvPersonas.DataSource = datos
            DgvPersonas.DataMember = "quipux_circular"
...
#Region "INICIALIZACION"
    Private Sub circular_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#Region "DATOS A CAJAS DE TEXTO"
    Private Sub DgvPersonas_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DgvPersonas.CellContentClick
        ...
        txt_circular.Text = DgvPersonas(1, i).Value
...
#Region "MOSTRAR DATOS"
    ...
    Public Sub MostrarDatos()
        consulta("SELECT * FROM quipux_circular", "quipux_circular")
        DgvPersonas.DataSource = datos.Tables("quipux_circular")
    End Sub
#End Region

#Region "REGISTRAR"
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If (txt_asunto.Text = "") Or (txt_circular.Text = "") Or (txt_fecha.Text = "") Then
            MsgBox("Campos Vacios", MsgBoxStyle.Information, "Confirmar")
        Else
            Try
                Dim consulta5 As String = "SELECT * FROM quipux_circular WHERE CIRCULAR_NUMERO = '" & txt_circular.Text & "'"
                Dim ADAPTADORC As New MySqlDataAdapter(consulta5, conexion1)
                Dim datos As New DataSet
                ADAPTADORC.Fill(datos, "quipux_circular")
                lista = datos.Tables("quipux_circular").Rows.Count
                If lista <> 0 Then
                    MsgBox("EL CODIGO " & txt_circular.Text & " YA EXISTE EN LA BASE DE DATOS", MsgBoxStyle.Critical, "Error")
                    'BorrarCampos()
                    txt_numero.Text = ""
                    txt_circular.Text = ""
                    txt_asunto.Text = ""
                ...
                Else
                    Dim ENTRADA As String = "INSERT INTO quipux_circular (CIRCULAR_NUMERO, ASUNTO, DIRIGIDO, UNIDAD_ACADEMICA, FIRMADO_POR, FECHA_DE_ELABORACION) VALUES ('" + txt_circular.Text + "', '" + txt_asunto.Text + "', '" + txt_dirigido.Text + "', '" + txt_unidad.Text + "', '" + txt_firma.Text + "', '" + txt_fecha.Text + "')"
...
#Region "MODIFICAR DATOS"
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        Else
            Dim comando As New MySqlCommand("UPDATE quipux_circular Set CIRCULAR_NUMERO = '" & txt_circular.Text & "', ASUNTO = '" & txt_asunto.Text & "', DIRIGIDO = '" & txt_dirigido.Text & "', UNIDAD_ACADEMICA = '" & txt_unidad.Text & "', FIRMADO_POR = '" & txt_firma.Text & "', FECHA_DE_ELABORACION = '" & txt_fecha.Text & "' WHERE NUMERO = '" & txt_numero.Text & "'", conexion1)
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        ...
        Else
            ...
            If respuesta = "123" Then
                Dim comando As New MySqlCommand("DELETE FROM quipux_circular WHERE NUMERO ='" & txt_numero.Text & "'", conexion1)
                comando.ExecuteNonQuery()
...
#Region "Alerta de seguridad"
    Public Sub alerta()
        enviarCorreos(emisor:="pruebaapp.2624@gmail.com", password:="jvmb pdrf bcfb zoaw", mensaje:="Se a detectado el ingreso del usuario " + autenticacion.txt_usuario.Text + "  Al departamento de QUIPUX CIRCULAR", asunto:="Mensaje de alerta sistema SIFPAPV", destinatario:="carmen.rivadeneiraa@ug.edu.ec, sadafatuly@hotmail.com, saadda.fatulya@ug.edu.ec")
    End Sub
#End Region

#Region "BUSQUEDA NUMERO DE CIRCULAR"
    Private Sub NOMBREToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles NOMBREToolStripMenuItem.Click
        If conexion1.State = ConnectionState.Closed Then
            conexion1.Open()
        End If
        Dim ENTRADA1 As String = "SELECT *FROM quipux_circular WHERE CIRCULAR_NUMERO LIKE '%" & txt_busqueda.Text & "%'"
...
#Region "BUSQUEDA ASUNTO"
    Private Sub APELLIDOToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles APELLIDOToolStripMenuItem.Click
        If conexion1.State = ConnectionState.Closed Then
            conexion1.Open()
        End If
        Dim ENTRADA1 As String = "SELECT *FROM quipux_circular WHERE ASUNTO LIKE '%" & txt_busqueda.Text & "%'"
        Dim ADAPTADOR2 As New MySqlDataAdapter(ENTRADA1, conexion1)
        datos = New DataSet
        ADAPTADOR2.Fill(datos)
        If conexion1.State = ConnectionState.Open Then
